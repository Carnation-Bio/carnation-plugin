...
4. Preview representative fields, including controls when available. As each preview completes, fetch and present a representative source image and every relevant mask or overlay directly in the conversation, then fetch and summarize the relevant measurement rows. Do not leave the evidence hidden behind collapsed tool calls or ask the user to expand them. If the client cannot render native image content inline, state that client limitation once and continue with the quantitative evidence. A rendered image alone does not establish biological correctness.
